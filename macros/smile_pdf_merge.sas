...
/**
  @file
  @brief Merge multiple PDF files and create one bookmark entry per PDF file with PROC GROOVY and open-source Tool PDFBox
  @details Merge multiple PDF files and create one bookmark entry per PDF file with PROC GROOVY and open-source Tool PDFBox

  Make sure to download PDFBOX, e.g. from here https://pdfbox.apache.org/download.html - the full "app" version

  Issues

    @li "unable to resolve class" messages mean the PDFBOX is not provided correctly.
    @li "ERROR: PROCEDURE GROOVY cannot be used when SAS is in the lockdown state." means that your SAS environment does not support PROC GROOVY, for this the macro cannot run the groovy code.
    @li "WARNUNG: Removed /IDTree from /Names dictionary, doesn't belong there" - this message is coming from PDFBox.

  @remark
  A paper explaining how to use PDFBOX with PROC GROOVY also for TOC is available in the following paper (https://www.lexjansen.com/phuse/2019/ct/CT05.pdf)

  Example Program
  
        ../programs/test_smile_pdf_merge.sas
  ...
  @param data Input dataset containing INFILE and BOOKMARK variable, INFILE containing single pdf files (one file per observation), BOOKMARK contaiing the corresponding bookmark label for this file
  @param outfile Output PDF file (not in quotes)
  @param pdfbox_jar_path Path and jar file name for PDFBOX open source tool, e.g. &path/pdfbox-app-2.0.22.jar
  @param sourcefile Optional sas program file where PROC GROOVY code is stored, default is TEMP (only temporary)
  @run_groovy NO/YES indicator whether to run the final GROOVY code (default YES)
